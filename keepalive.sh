...
# default values:
# cpu: 20%
# memory: 1/5(just work on arm instance)
# network: 6M/s
...
set_cpu_net () {
    cpuq=$(lscpu | awk '/^CPU\(/{print $NF*100/5}');
    ...
    while true;
    do
        curl -skLo /dev/null "${durl}" --limit-rate 6M;
    done
    wait
eof
    ...
    [Unit]
    Description=cpu stress 20 percents & download file with 6M/s speed
